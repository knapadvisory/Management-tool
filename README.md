...
- A shared team **Drive** — a common file library everyone can see. Upload files (button or drag-and-drop) and any teammate can open, preview or download them; it uses the same file-manager interface as Files (columns, sort, views, search, selection). You can only delete files **you** uploaded, and deletions land in the admin Archive just like Files
