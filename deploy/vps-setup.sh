...
# Continuous off-site DB replication (litestream). Set LITESTREAM_REPLICA_URL to
# an S3-compatible bucket (e.g. s3://my-bucket/teamhub) plus the two keys to
# stream every change off-box within seconds. LITESTREAM_ENDPOINT +
# LITESTREAM_FORCE_PATH_STYLE="true" are for non-AWS S3 (Backblaze B2, MinIO,
# Wasabi…). Leave blank to keep the on-box daily backups only. See DEPLOY.md.
LITESTREAM_REPLICA_URL="${LITESTREAM_REPLICA_URL:-}"
LITESTREAM_ACCESS_KEY_ID="${LITESTREAM_ACCESS_KEY_ID:-}"
LITESTREAM_SECRET_ACCESS_KEY="${LITESTREAM_SECRET_ACCESS_KEY:-}"
LITESTREAM_ENDPOINT="${LITESTREAM_ENDPOINT:-}"
LITESTREAM_FORCE_PATH_STYLE="${LITESTREAM_FORCE_PATH_STYLE:-}"
EOF
...
docker run -d --name teamhub --restart unless-stopped \
  ...
  ${LITESTREAM_REPLICA_URL:+-e LITESTREAM_REPLICA_URL="$LITESTREAM_REPLICA_URL"} \
  ${LITESTREAM_ACCESS_KEY_ID:+-e LITESTREAM_ACCESS_KEY_ID="$LITESTREAM_ACCESS_KEY_ID"} \
  ${LITESTREAM_SECRET_ACCESS_KEY:+-e LITESTREAM_SECRET_ACCESS_KEY="$LITESTREAM_SECRET_ACCESS_KEY"} \
  ${LITESTREAM_ENDPOINT:+-e LITESTREAM_ENDPOINT="$LITESTREAM_ENDPOINT"} \
  ${LITESTREAM_FORCE_PATH_STYLE:+-e LITESTREAM_FORCE_PATH_STYLE="$LITESTREAM_FORCE_PATH_STYLE"} \
  -v teamhub-data:/data \
  teamhub:latest
