...
import { RECURRENCES, occurrencesInRange, stepStart } from '../recurrence.js';

const router = Router();
const durationMs = (ev) => (ev.ends_at ? new Date(ev.ends_at.replace(' ', 'T') + 'Z') - new Date(ev.starts_at.replace(' ', 'T') + 'Z') : 0);
...
router.get('/', (req, res) => {
  ...
  // Pull every event that could touch the window (recurring ones start earlier),
  // then expand each into the occurrences that fall inside it.
  const rows = db.prepare(`
    SELECT * FROM calendar_events
    WHERE user_id = ? AND starts_at <= ? AND (recurrence != 'none' OR COALESCE(ends_at, starts_at) >= ?)
    ORDER BY starts_at
  `).all(req.user.id, to, from);
  const events = [];
  for (const ev of rows) {
    const dur = durationMs(ev);
    for (const start of occurrencesInRange(ev, from, to)) {
      const ends = !ev.all_day && dur ? new Date(new Date(start.replace(' ', 'T') + 'Z').getTime() + dur).toISOString().slice(0, 19).replace('T', ' ') : (ev.recurrence === 'none' ? ev.ends_at : null);
      events.push({ ...ev, series_start: ev.starts_at, starts_at: start, ends_at: ends, occurrence: ev.recurrence !== 'none' });
    }
  }
...
router.post('/', (req, res) => {
  ...
  const repeatUntil = recurrence !== 'none' && b.repeat_until ? String(b.repeat_until).slice(0, 10) : null;
  const info = db.prepare(`
    INSERT INTO calendar_events (workspace_id, user_id, title, starts_at, ends_at, all_day, notes, color, remind_min, recurrence, repeat_until)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(req.workspaceId, req.user.id, title, starts, ends, allDay, String(b.notes || '').slice(0, 4000), String(b.color || '').slice(0, 20), remind, recurrence, repeatUntil);
...
router.patch('/:id', (req, res) => {
  ...
  if (b.remind_min !== undefined) { sets.push('remind_min = ?', 'reminded = 0', 'reminded_occurrence = NULL'); vals.push(b.remind_min === '' || b.remind_min == null ? null : Math.max(0, Math.min(10080, parseInt(b.remind_min, 10) || 0))); }
  if (b.recurrence !== undefined) { sets.push('recurrence = ?'); vals.push(RECURRENCES.includes(b.recurrence) ? b.recurrence : 'none'); }
  if (b.repeat_until !== undefined) { sets.push('repeat_until = ?'); vals.push(b.repeat_until ? String(b.repeat_until).slice(0, 10) : null); }
